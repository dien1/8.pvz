...
enum {WAN_DAO,XIANG_RI_KUI,SHI_REN_HUA,YING_TAO_ZHA_DAN,JIAN_GUO_QIANG,ZHI_WU_COUNT};	//植物枚举
...
void gameInit() {
	...
	memset(imgZhiWu, 0, sizeof(imgZhiWu));	//给指针赋空值，imgzhiwu是一个指针数组
...
void updateWindow() {
	...
	if (curZhiWu > 0) {
		...
		if (img != nullptr) {
			putimagePNG(curX - img->getwidth() / 2, curY - img->getheight() / 2, img);//光标在图片中心
		}
		else {
			// 处理 img 为 NULL 的情况
		}
	...
	if (peekmessage(&msg)) {	//判断用户是否有操作
		if (msg.message == WM_LBUTTONDOWN) {	//鼠标左键按下
			if (msg.x > 343 && msg.x < 343 + 65 * ZHI_WU_COUNT && msg.y < 96) {	//点击卡牌的事件
				...
				else if (index == YING_TAO_ZHA_DAN) {
					if (sunShine >= 150) {
						status = 1;
						curZhiWu = index + 1;
						//使植物显示在点击位置，避免了植物出现在上次消失位置的小bug
				...
				//status = 1;//使植物显示在点击位置，避免了植物出现在上次消失位置的小bug
				//curX = msg.x;
				//curY = msg.y;
			}
			else {	//收集阳光事件
		...
		else if (msg.message == WM_LBUTTONUP && status == 1) {	//鼠标右键按下
